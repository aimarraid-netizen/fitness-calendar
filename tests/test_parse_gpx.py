...
def test_z2_minutes_computed():
    data = parse_gpx.parse_gpx(SAMPLE)
    # kõik sämplid 135-140 bpm = z2 (60/180 puhul z2 = 132..144)
...
def test_insert_dedup_and_cardio_cols_persisted(conn):
    data = parse_gpx.parse_gpx(SAMPLE)
    ...
    assert row["timestamp"] == "2026-06-01 21:00:00"   # 18:00Z -> lokaalaeg
    assert row["z2_min"] == 10.0
    assert row["z1_min"] == 0.0 and row["z3_min"] == 0.0
    assert row["max_hr"] == 140
    assert row["ascent_m"] == 8
    assert row["workout_type"] == "kõndimine"
    assert row["workout_name"] == "Õhtune kõnd"


def test_fit_and_gpx_of_same_activity_dedup_across_sources(conn):
    # sama algushetk erinevast allikast (nt Garmin FIT + Strava GPX) = üks trenn
    data = parse_gpx.parse_gpx(SAMPLE)
    _, wid = parse_gpx.insert_workout(conn, SAMPLE, data)
    import cardio_common
    assert cardio_common.find_existing_cardio(conn, "2026-06-01 21:00:00")["id"] == wid
